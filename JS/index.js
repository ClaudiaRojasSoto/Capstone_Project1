...
function updateCardLayout() {
  ...
  speakersData.forEach((speaker, index) => {
    ...
    if (window.innerWidth >= 769 || index < 2) {
      if (window.innerWidth >= 769) { // Versión desktop
        card.style.width = window.innerWidth >= 769 ? '50%' : '100%';
        card.style.paddingLeft = window.innerWidth >= 769 ? '0' : '10px';
      ...
      featuredSpeakersSection.appendChild(card);
    }
  });
